...
export async function runDownload(onProgress) {
  // ── 1. Repository default branch + latest commit ────────────────────────
  onProgress(5, 100, 'Fetching repository info...')
  const repoInfo = await fetch(GITHUB_API, {
  ...
  const branchTip = await fetch(`${GITHUB_API}/commits/${encodeURIComponent(defaultBranch)}`, {
    headers: { Accept: 'application/vnd.github+json' },
  }).then(r => { if (!r.ok) throw new Error(`Branch commit ${r.status}`); return r.json() })

  const ref = branchTip.sha
  if (!ref || typeof ref !== 'string' || !/^[a-f0-9]{40}$/i.test(ref)) {
    throw new Error(`Unexpected commit SHA format: ${String(ref).slice(0, 40)}`)
  }
  logger.info('download', `Using ${defaultBranch}@${ref.slice(0, 12)}`)

  // ── 2. Sets (single file) ───────────────────────────────────────────────
  onProgress(10, 100, 'Fetching sets...')
  const setsData = await fetch(`${GITHUB_RAW}/${encodeURIComponent(ref)}/sets/en.json`)
  ...
  const contents = await fetch(`${GITHUB_API}/contents/cards/en?ref=${encodeURIComponent(ref)}`, {
    headers: { Accept: 'application/vnd.github+json' },
  ...
  for (let i = 0; i < cardFiles.length; i += CARD_BATCH_SIZE) {
    ...
    const results = await Promise.all(
      batch.map(file =>
        fetch(`${GITHUB_RAW}/${encodeURIComponent(ref)}/cards/en/${encodeURIComponent(file)}`)
          .then(r => r.ok ? r.json() : [])
          .catch(() => [])
      )
    )
