...
### Database Management
- **Functionality**: Downloads and caches the complete Pokemon TCG card database from the PokemonTCG/pokemon-tcg-data repository tip on-demand with progress tracking, storing all card metadata locally
- **Purpose**: Enables offline card recognition and provides accurate, up-to-date card information without relying on release publishing delays
- **Trigger**: Automatic on first launch if no database exists, or manual via database button in header
- **Progression**: Click database icon → View database status → Tap refresh → Resolve latest repository commit → Download and parse set/card JSON files → Database cached locally → Success notification
- **Success criteria**: Full database downloads successfully from the repository tip, imports all sets and cards, all cards searchable, and lookup performs within 100ms
